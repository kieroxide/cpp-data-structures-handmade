...
class Hash
{
   private:
    ...
    // Adds key and value to hashmap. No resizing
    // Time Complexity:
    // Average: O(1)
    // Worst-case (due to collisions or resizing): O(n)
    void putNoResize(int key, Type value) {
        // Locates key's index
    ...
    void resize() {
        ...
        size_t oldCapacity = buckets.getCapacity();
        ...
        for (size_t i = 0; i < pairs.getSize(); i++) {
            putNoResize(pairs[i].key, pairs[i].value);
        }
    }
    ...
    Vector<Pair<int, Type>> getAllPairs() {
        ...
        for (size_t i = 0; i < buckets.getSize(); i++) {
            ...
            for (size_t k = 0; k < bucket.getSize(); k++) {
   ...
   public:
    ...
    friend std::ostream& operator<<(std::ostream& os, const Hash& hash) {
        os << "[";
        for (size_t i = 0; i < hash.buckets.getSize(); i++) {
            os << hash.buckets[i];
